...
void render::setup() {
	...
	fonts[rolexBOLD] = create_font("Segoe UI Emoji", 12, FW_BOLD, FONTFLAG_NONE);
	fonts[ESPFLAG] = create_font(/*"Open Sans Semibold"*/"Verdana", 12, FW_HEAVY, FONTFLAG_NONE | FONTFLAG_ANTIALIAS);
	fonts[TAHOMA12] = create_font("Tahoma", 12, FW_NORMAL, FONTFLAG_NONE);
	fonts[MENUNAME] = create_font("Segoe UI Emoji", 15, FW_THIN, FONTFLAG_NONE);
	fonts[VERDANA12] = create_font("Segoe UI Emoji", 12, FW_NORMAL, FONTFLAG_NONE);
	fonts[VERDANA12OUT] = create_font("Segoe UI Emoji", 12, FW_NORMAL, FONTFLAG_NONE);
	fonts[VERDANA12BOLD] = create_font("Segoe UI Emoji", 12, FW_BOLD, FONTFLAG_ANTIALIAS | FONTFLAG_DROPSHADOW);
	fonts[VERDANA12BOLDOUT] = create_font("Segoe UI Emoji", 12, FW_BOLD, FONTFLAG_ANTIALIAS | FONTFLAG_NONE);
	fonts[ESPNAME] = create_font(/*"Open Sans Semibold"*/"Verdana", 11, FW_HEAVY, FONTFLAG_NONE | FONTFLAG_ANTIALIAS);
	fonts[ESPHEALTH] = create_font("Segoe UI Emoji", 12, FW_THIN, FONTFLAG_NONE);
	fonts[WEAPONICONS] = create_font("astriumwep", 10, FW_MEDIUM, FONTFLAG_ANTIALIAS | FONTFLAG_DROPSHADOW);
	fonts[GRENADEICONS] = create_font("undefeated", 21, FW_MEDIUM, FONTFLAG_ANTIALIAS | FONTFLAG_DROPSHADOW);
	fonts[BOMBICONS] = create_font("astriumwep", 21, FW_MEDIUM, FONTFLAG_ANTIALIAS | FONTFLAG_DROPSHADOW);
	...
	fonts[NYAFONT] = create_font("Return of the Grid", 12, FW_NORMAL, FONTFLAG_NONE);
	...
	fonts[ICONS] = create_font("Aurora", 40, FW_MEDIUM, FONTFLAG_DROPSHADOW);
	...
	fonts[keybind_font] = create_font("Verdana", 8, 700, FONTFLAG_DROPSHADOW);
	fonts[CTAB_MIN] = create_font("Verdana", 12, 700, FONTFLAG_DROPSHADOW);
	fonts[CATICON] = create_font("blobtabs", 12, FW_NORMAL, FONTFLAG_DROPSHADOW);
	fonts[weapons_big] = create_font("astriumwep", 25, FW_NORMAL, FONTFLAG_DROPSHADOW);
}
...
void render::draw_text(float x, float y, const char* msg, Color color)
{
	color.SetAlpha(static_cast<int>(color.a() * alpha_factor));
	...
	int width, height;
	g_csgo.m_surface()->GetTextSize(fonts[CTAB], wbuf, width, height);

	g_csgo.m_surface()->DrawSetTextFont(CTAB);
...
void render::draw_text_stdstring(int x, int y, unsigned long font, std::string string, bool text_centered, colorc colour) {
	std::wstring text = std::wstring(string.begin(), string.end());
	const wchar_t* converted_text = text.c_str();

	int width, height;
	g_csgo.m_surface()->GetTextSize(font, converted_text, width, height);

	g_csgo.m_surface()->DrawSetTextColor(colour.r, colour.g, colour.b, colour.a);
	g_csgo.m_surface()->DrawSetTextFont(font);
	if (text_centered)
		g_csgo.m_surface()->DrawSetTextPos(x - (width / 2), y);
	else
		g_csgo.m_surface()->DrawSetTextPos(x, y);
	g_csgo.m_surface()->DrawPrintText(converted_text, wcslen(converted_text));
}
