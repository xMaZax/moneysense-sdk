...
void C_MultiDropdown::draw() {
	if (menu::get().activate_menu)
	{
		...
		if (label != "")
			render::get().text(fonts[CTAB], area.x, area.y, Color::White, HFONT_CENTERED_NONE, label.c_str());
		else
			render::get().text(fonts[CTAB], area.x, area.y, Color::White, HFONT_CENTERED_NONE, "...");

		POINT mouse; GetCursorPos(&mouse);
		rect_t item_region = rect_t(n_area.x, n_area.y, n_area.w, gg);


		render::get().rect_filled(n_area.x, n_area.y, n_area.w, gg, Color{ 55, 55, 55, 255 });
		...
		render::get().text(fonts[WATERMARK], n_area.x + 10, n_area.y + 10, Color(153, 153, 153), HFONT_CENTERED_Y, "%s", ss.c_str());
		...
		for (int i = 0; i < items.size(); i++) {
			...
			if (current.enabled) {
				...
				render::get().text(fonts[CTAB], area.x + 10, n_area.y + (gg * i) + 10, g_cfg.menu.menu_theme[0], HFONT_CENTERED_Y, current.name.c_str());
			}
			else {
				render::get().text(fonts[WATERMARK], area.x + 10, n_area.y + (gg * i) + 10, Color(153, 153, 153), HFONT_CENTERED_Y, current.name.c_str());
			}
		}
