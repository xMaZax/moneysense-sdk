...
void bullettracers::events(IGameEvent * event) {
	if (!strcmp(event->GetName(), "bullet_impact")) {
		...
		{
			if (g_cfg.esp.bullet_tracer_enemy)
			{
				auto player = static_cast<player_t *>(g_csgo.m_entitylist()->GetClientEntity(index));
				if (player->IsDormant() || player->m_iTeamNum() == g_ctx.m_local->m_iTeamNum())
					return;
				logs.push_back(trace_info(player->get_eye_pos(), position, g_csgo.m_globals()->m_curtime));

				Color color = g_cfg.esp.bullet_tracer_color_enemy;
				draw_beamenemy(player->get_eye_pos(), position, color);
			}
		}
	}
}