...
void __fastcall hooks::hooked_sceneend(void * thisptr, void * edx) {
	...
	if (g_cfg.esp.ghost_chams && g_ctx.m_local->is_alive()) {
		...
		g_csgo.m_modelrender()->ForcedMaterialOverride(materials[3]);
	...
	if (g_cfg.esp.local_chams && g_ctx.m_local->is_alive()) {
		...
		g_csgo.m_renderview()->SetBlend(0.6f);
	...
	static IMaterial* materials_hand[3] = {
	g_csgo.m_materialsystem()->FindMaterial("models/inventory_items/trophy_majors/crystal_clear", TEXTURE_GROUP_MODEL),
	g_csgo.m_materialsystem()->FindMaterial("models/inventory_items/trophy_majors/gloss", TEXTURE_GROUP_MODEL),
	};

	if (g_cfg.esp.handchams == 0)
	{

	}
	if (g_cfg.esp.handchams == 1)
	{
		g_csgo.m_modelrender()->ForcedMaterialOverride(materials_hand[1]);

		g_ctx.m_local->DrawModel(0x1, 255);

		g_csgo.m_modelrender()->ForcedMaterialOverride(nullptr);
	}
	else if (g_cfg.esp.handchams == 2)
	{
		g_csgo.m_modelrender()->ForcedMaterialOverride(materials_hand[2]);

		g_ctx.m_local->DrawModel(0x1, 255);

		g_csgo.m_modelrender()->ForcedMaterialOverride(nullptr);
	}
	else if (g_cfg.esp.handchams == 3)
	{
		g_csgo.m_modelrender()->ForcedMaterialOverride(materials_hand[3]);
...
#define clamp(val, min, max) (((val) > (max)) ? (max) : (((val) < (min)) ? (min) : (val)))
	...
	if (g_ctx.m_local->m_vecVelocity().Length2D() > 30 && g_ctx.m_local && g_cfg.esp.fake_chams && g_ctx.m_local->is_alive())
	{
		static auto last_networked_origin = Vector(0, 0, 0);
		if (g_ctx.send_packet)
			last_networked_origin = g_ctx.m_local->m_vecOrigin();
		...
		float clr[4] = {
		g_cfg.esp.fake_chams_color[0] / 255.f,
	    g_cfg.esp.fake_chams_color[1] / 255.f,
	    g_cfg.esp.fake_chams_color[2] / 255.f,
		g_cfg.esp.fake_chams_color[3] / 255.f
		};

		g_csgo.m_renderview()->SetColorModulation(clr);
		doggy->ColorModulate(g_cfg.esp.fake_chams_color[0], g_cfg.esp.fake_chams_color[1], g_cfg.esp.fake_chams_color[3]);
