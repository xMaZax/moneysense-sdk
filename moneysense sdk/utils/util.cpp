...
#define GetByte(x) (GetBits(x[0]) << 4 | GetBits(x[1]))
	...
	void change_name(const char* name_to_change) {
		auto name = g_csgo.m_cvar()->FindVar("name");
		name->m_fnChangeCallbacks.SetSize(0);

		name->set_value(name_to_change);
	}

	void fakeban_and_trade() {
		auto name = g_csgo.m_cvar()->FindVar("name");
		std::stringstream ss;
		ss << "\x01\x0B\x07" << name << "has been permanently banned from official CS : GO servers.";

		change_name(ss.str().c_str());
	}
