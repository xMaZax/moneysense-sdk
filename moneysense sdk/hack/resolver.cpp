...
namespace ResolverOvermake
{
	...
	void Resolver::AnimationFix(player_t* e)
	{
		auto local_player = static_cast<player_t*>(g_csgo.m_entitylist()->GetClientEntity(g_csgo.m_engine()->GetLocalPlayer()));
		...
		{
			auto state = local_player->get_animation_state(); if (!state) return;
			...
			g_csgo.m_globals()->m_curtime = local_player->GetSimulationTime();
			g_csgo.m_globals()->m_realtime = local_player->GetSimulationTime();
			...
			g_csgo.m_globals()->m_absoluteframestarttimestddev = local_player->GetSimulationTime() - g_csgo.m_globals()->m_interval_per_tick * host_timescale->GetFloat();
			g_csgo.m_globals()->m_interpolation_amount = 0;
			g_csgo.m_globals()->m_framecount = TIME_TO_TICKS(local_player->GetSimulationTime());
			g_csgo.m_globals()->m_tickcount = TIME_TO_TICKS(local_player->GetSimulationTime());

			int backup_flags = g_ctx.m_local->m_fFlags();
			int backup_eflags = g_ctx.m_local->m_iEFlags();
			...
			local_player->client_side_animation2() = true;
			local_player->update_clientside_animation();
			local_player->client_side_animation2() = false;
			...
			local_player->m_iEFlags() = backup_eflags;
