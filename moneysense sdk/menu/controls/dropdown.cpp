...
void C_Dropdown::draw() {
	if (menu::get().activate_menu)
	{
		...
		render::get().text(fonts[WATERMARK], n_area.x + 10, n_area.y + 10, Color(153, 153, 153), HFONT_CENTERED_Y, items[*selected].c_str());
		...
		for (int i = 0; i < items.size(); i++) {
			...
			if (i == *selected) {
				...
				render::get().text(fonts[CTAB], area.x + 10, n_area.y + (gg * i) + 10, Color(153, 153, 153), HFONT_CENTERED_Y, current.c_str());
			}
		}
