...
void eventlogs::events(IGameEvent * event) {
	...
	if (g_cfg.misc.events_to_log[EVENTLOG_HIT].enabled && strstr(event->GetName(), "player_hurt")) {
		...
		//	cvar->ConsoleColorPrintf(Color(108, 195, 18), "[moneysense]");
		//	cvar->ConsoleDPrintf(" ""hit"" ");
		//	cvar->ConsoleColorPrintf(Color(255, 255, 255), "%s", userid_info.szName); //look into this TODO:
		//	cvar->ConsoleColorPrintf(Color(255, 255, 255), " in the %s", get_hitgroup_name(event->GetInt("hitgroup")));
		//	cvar->ConsoleDPrintf(" ""for"" ");
		//	cvar->ConsoleColorPrintf(Color(255, 255, 255), "%s", dmg_to_health);
		//	cvar->ConsoleColorPrintf(Color(255, 255, 255), " (%s remaining)\n", remaining_health);
