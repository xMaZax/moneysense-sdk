	...
	if (panel_id == panel) {
		if (g_ctx.available() && local_player) {
			...
			if (g_cfg.esp.removescope && local_player->m_bIsScoped()) {
				...
				g_csgo.m_engine()->GetScreenSize(width_2, height_2);
				auto weapon = local_player->m_hActiveWeapon();

				if (!weapon)
					return;

				float spread = weapon->get_innacuracy() * 100;
				int height = std2017::clamp(spread, 1.f, 30.f);
				int alpha = 255 - (height * 4.2f);

				bool Dynamic = g_cfg.esp.removescope;

				if (Dynamic) {
					render::get().rect_filled(0, height_2 / 2 - (height / 2), width_2, height, Color(0, 0, 0, alpha));
					render::get().rect_filled(width_2 / 2 - (height / 2), 0, height, height_2, Color(0, 0, 0, alpha));
				}
				else {
					render::get().line(0, height_2 / 2, width_2, height_2 / 2, Color(0, 0, 0, 255));
					render::get().line(width_2 / 2, 0, width_2 / 2, height_2, Color(0, 0, 0, 255));
				}
				draw::autoskope();
			}
			...
			otheresp::get().check_hits();
			otheresp::get().status_bar();
			if (g_csgo.m_inputsys()->IsButtonDown(g_cfg.ragebot.baim_bind))
				otheresp::get().baim_indicator();
			if (g_cfg.esp.indicators[DESYNC].enabled) {
		...
		if (g_cfg.ragebot.resolver_experimental) {
			render::get().gradient(0, 0, 4000, 3, Color(51, 132, 255, 255), Color(235, 29, 173, 255), GRADIENT_HORIZONTAL);
			render::get().rect_filled(0, 2, 4000, 1, Color(0, 0, 0, 155));
		}
